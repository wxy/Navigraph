...
export function registerUIHandlers(messageService: ContentMessageService): void {
  ...
  messageService.registerHandler('refreshVisualizer', (
    ...
    try {
      logger.log(_('ui_handlers_refresh_request', '处理刷新可视化器请求'));
      ...
      if (visualizer && typeof visualizer.triggerRefresh === 'function') {
        ...
        logger.warn(_('ui_handlers_visualizer_unavailable', '可视化器不可用，无法刷新'));
        ctx.error(_('ui_handlers_visualizer_error', '可视化器不可用'));
      }
      return false; // 同步响应
    } catch (error) {
      logger.error(_('ui_handlers_refresh_error', '刷新可视化器失败: {0}'), 
        error instanceof Error ? error.message : String(error));
      ctx.error(_('ui_handlers_refresh_error', '刷新可视化器失败: {0}'), 
        error instanceof Error ? error.message : String(error));
      return false; // 同步响应
    }
  });
  
  logger.log(_('ui_handlers_registered', 'UI相关消息处理程序已注册'));
}