...
import {
  ...
  (function() {
    ...
    function safeSendMessage<
      ...
      TResponse extends BaseResponseMessage = BaseResponseMessage
    >(
      message: TMessage,
      ...
      try {
        if (callback) {
          chrome.runtime.sendMessage(message, function(response: TResponse) {
     ...
     */
    function requestNodeId(url: string, retryCount: number = 0): void {
      if (!url) return;
      ...
      // 发送请求到后台脚本
      const message: GetNodeIdRequestMessage = {
        action: 'getNodeId',
        url: url
      };
      
      safeSendMessage<GetNodeIdRequestMessage, GetNodeIdResponseMessage>(
        message, 
        response => {
    ...
    function sendPageLoadedMessage(): void {
      ...
      // 强制转换ancestorOrigins为字符串数组
      const ancestorOrigins: string[] = Array.from(
        (location.ancestorOrigins || []) as unknown as Iterable<string>
      );
      ...
      const message: PageLoadedRequestMessage = {
        action: 'pageLoaded',
        pageInfo: pageInfo as any // 注意：这里需要类型断言，因为类型定义中的pageInfo结构与此不完全匹配
      };
      
      safeSendMessage<PageLoadedRequestMessage, PageLoadedResponseMessage>(
        message, 
        function(response) {
    ...
    function setupLinkClickListener(): void {
      document.addEventListener('click', function(e) {
        ...
        // 查找被点击的链接元素
        let linkElement: HTMLElement | null = e.target as HTMLElement;
        ...
        if (linkElement && (linkElement as HTMLAnchorElement).href) {
          const anchorElement = linkElement as HTMLAnchorElement;
          const isNewTab = anchorElement.target === '_blank' || 
                           e.ctrlKey || 
                           e.metaKey || 
                           (e as MouseEvent).button === 1;
          try {
            const message: LinkClickedRequestMessage = {
              action: 'linkClicked',
              linkInfo: {
            ...
            console.log(`链接点击: ${window.location.href} -> ${anchorElement.href} `);
          } catch (e) {
    ...
    function setupJsNavigationListener(): void {
      ...
      history.pushState = function(
        ...
        if (standardNodeId && isExtensionActive && url) {
          const targetUrl = url.toString(); // 新URL
