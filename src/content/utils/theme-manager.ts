...
export class ThemeManager {
  ...
  public async initialize(): Promise<void> {
    // 防止重复初始化
    if (this.initialized) {
      logger.log(_('theme_manager_already_initialized', '主题管理器已初始化，跳过'));
      return;
    }
    ...
    try {
      ...
      if (window.navigraphSettings && window.navigraphSettings.theme) {
        ...
        logger.log(_('theme_manager_read_global_config', '从全局配置读取主题设置: {0}'), theme);
      } else {
        // 尝试从设置服务获取
        theme = this.settingsService.getSetting("theme") || 'system';
        logger.log(_('theme_manager_read_settings_service', '从设置服务读取主题设置: {0}'), theme);
      }
      
      // 在初始化时应用主题
      this.applyTheme(theme);
    } catch (error) {
      logger.warn(_('theme_manager_settings_failed', '获取主题设置失败，使用系统主题: {0}'), error instanceof Error ? error.message : String(error));
      this.applyTheme("system");
    }
  
    logger.log(_('theme_manager_init_complete', '主题管理器初始化完成，当前主题: {0}'), this.currentTheme);
  }
  ...
  public applyTheme(theme: Theme): void {
    logger.log(_('theme_manager_applying_theme', '应用主题设置: {0}'), theme);
  ...
  private applySpecificTheme(theme: Theme): void {
    ...
    if (this.currentTheme === theme) {
      logger.log(_('theme_manager_no_change_needed', '主题已经是 {0}，无需更改'), theme);
      return;
    }

    logger.log(_('theme_manager_setting_theme', '设置主题为: {0}'), theme);
    ...
    logger.log(_('theme_manager_theme_updated', '已更新主题为: {0}'), theme);
  }
  ...
  public getThemeFromLocalStorage(): Theme | null {
    try {
      ...
      logger.warn(_('theme_manager_localstorage_get_failed', '从本地存储获取主题失败: {0}'), error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  private saveThemeToLocalStorage(theme: Theme): void {
    try {
      ...
      logger.warn(_('theme_manager_localstorage_save_failed', '保存主题到本地存储失败: {0}'), error instanceof Error ? error.message : String(error));
    }
  }
  ...
  private ensureStylesheetLoaded(): void {
    ...
    if (!document.querySelector('link[href*="themes.css"]')) {
      ...
      logger.log(_('theme_manager_stylesheet_added', '已添加主题样式表'));
    }
  }
