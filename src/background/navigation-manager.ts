...
export class NavigationManager {
  ...
  private pendingJsNavigations = new Map<number, { from: string, to: string }[]>(); // 标签页ID -> JS导航记录
  ...
  private urlToNodeCache = new Map<string, {nodeId: string, timestamp: number}>(); // URL -> {节点ID, 时间戳}
  ...
  private async handleTabUpdated(
    ...
    try {
      ...
      if (this.debugMode) {
        console.log(`标签页更新: ID=${tabId}, 标题=${changeInfo.title || '没变'}, 图标=${changeInfo.favIconUrl ? '已更新' : '没变'}`);
      }
  ...
  private async handleTabActivated(activeInfo: chrome.tabs.TabActiveInfo): Promise<void> {
    ...
    try {
      ...
      if (previousActiveTime) {
        const activeTabId = Array.from(this.tabActiveTimes.entries())
          .find(([id, time]) => time === previousActiveTime && id !== tabId)?.[0];
      ...
      if (tabState) {
        ...
        try {
          ...
          if (this.debugMode) {
            console.log(`标签页激活: ID=${tabId}, 窗口=${windowId}, URL=${tab.url || '未知'}`);
          }
        } catch (err) {
          console.warn(`获取标签页信息失败: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    } catch (error) {
  ...
  private async handleTabRemoved(tabId: number, removeInfo: chrome.tabs.TabRemoveInfo): Promise<void> {
    try {
      ...
      if (this.debugMode) {
        console.log(`标签页关闭: ID=${tabId}, 窗口=${removeInfo.windowId}, 窗口关闭=${removeInfo.isWindowClosing}`);
      }
    } catch (error) {
  ...
  private async handleNavigationCommitted(
    ...
    try {
      // 如果不是主框架或是错误页面，忽略此事件
      if (details.frameId !== 0 || this.isErrorPage(details.url)) {
        return;
      }
      ...
      } else if (transitionType === 'generated' || transitionType === 'auto_subframe') {
      ...
      if (this.debugMode) {
        console.log(`导航提交: 标签页=${tabId}, URL=${url}, 类型=${navigationType}, 目标=${openTarget}`);
        console.log(`  - 过渡类型: ${transitionType}`);
        console.log(`  - 过渡限定词: ${transitionQualifiers.join(', ') || '无'}`);
      }
  ...
  private async handleRegularNavigation(
    ...
    try {
      ...
      if (pendingNav) {
        ...
        if (this.debugMode) {
          console.log(`找到匹配的待处理导航: ${pendingNav.type}, 父节点ID: ${parentId}`);
        }
      }
      ...
      if (this.debugMode) {
        console.log(`已创建导航节点: ID=${nodeId}, 父节点=${parentId || '无'}, 类型=${navigationType}`);
      }
    } catch (error) {
  ...
  private async handleHistoryStateUpdated(
    ...
    try {
      ...
      if (existingNodeId) {
        if (this.debugMode) {
          console.log(`历史状态更新: 已存在节点 ${existingNodeId} 于URL=${url}`);
        }
      ...
      if (!parentId) {
        if (this.debugMode) {
          console.log(`历史状态更新: 未找到父节点, 标签页=${tabId}, URL=${url}`);
        }
        return;
      }
      ...
      if (this.debugMode) {
        console.log(`历史状态更新: 已创建节点 ${nodeId}, 父节点=${parentId}, URL=${url}`);
      }
    } catch (error) {
  ...
  private async handleInitialNavigation(
    ...
    try {
      ...
      if (this.debugMode) {
        console.log(`已创建初始导航节点: ID=${nodeId}, 父节点=${parentNodeId || '无'}`);
      }
...
private async cleanupPendingUpdates(): Promise<void> {
  ...
  public handleLinkClicked(tabId: number, linkInfo: any): void {
    ...
    if (this.debugMode) {
      console.log(`链接点击: ${linkInfo.sourceUrl} -> ${targetUrl} (源节点: ${linkInfo.sourcePageId})`);
    }
  }
  ...
  public handleFormSubmitted(tabId: number, formInfo: any): void {
    ...
    if (this.debugMode) {
      console.log(`表单提交: ${formInfo.sourceUrl} -> ${formInfo.formAction} (源节点: ${formInfo.sourcePageId})`);
    }
  }
  ...
  public handleJsNavigation(tabId: number, message: any): void {
    ...
    if (this.debugMode) {
      console.log(`JS导航: ${message.sourceUrl} -> ${targetUrl} (源节点: ${message.sourcePageId})`);
    }
  }
  ...
  public async updateNodeMetadata(
    ...
    try {
      ...
      // Favicon处理 - 应用优先级策略
      if (metadata.favicon) {
        const useFavicon = !record.favicon || 
                          (source === 'content_script' && record.favicon.includes('google.com/s2/favicons')) ||
                          (source === 'chrome_api' && record.source === 'navigation_event');
        
        if (useFavicon) {
          updates.favicon = metadata.favicon;
          if (this.debugMode) {
            console.log(`更新Favicon: ${record.favicon ? '已有图标' : '无图标'} -> 新图标`);
          }
        }
      }
        ...
        if (this.shouldUseReferrerForParent(record)) {
          ...
          if (potentialParentId && potentialParentId !== nodeId) {
            if (!await this.wouldCreateCycle(potentialParentId, nodeId)) {
              ...
              if (this.debugMode) {
                console.warn(`基于引用信息的父节点 ${potentialParentId} -> ${nodeId} 会导致循环，已阻止`);
              }
            }
          }
        }
      ...
      if (metadata.loadTime && !record.loadTime) updates.loadTime = metadata.loadTime;
   ...
   * 从内容脚本请求获取元数据
   */
  public async updatePageMetadata(tabId: number, metadata: any): Promise<string | null> {
    if (!metadata || !tabId) {
      return null;
    }
   ...
   * 获取标签页的节点ID
   */
  public async getNodeIdForTab(tabId: number, url: string): Promise<string | null> {
    // 1. 首先尝试从缓存获取
    ...
    // 3. 最后尝试找标签页状态的最后节点
    const tabState = this.tabStates.get(tabId);
    if (tabState && tabState.lastNodeId && tabState.url && this.isSameUrl(tabState.url, url)) {
  ...
  private async findNodeByUrl(url: string): Promise<string | null> {
    ...
    try {
      ...
      if (cached && (Date.now() - cached.timestamp) < 60000) { // 1分钟内的缓存有效
      ...
      // 如果没找到，尝试标准化URL匹配
      if (!matchingRecord) {
        matchingRecord = records.find(r => this.normalizeUrl(r.url) === normalizedUrl);
      }
   ...
   * 更新标签页活跃时间
   */
  private async updateTabActiveTime(tabId: number, elapsedTime: number): Promise<void> {
    try {
      // 查找活跃标签页中的所有节点
      const history = this.tabNavigationHistory.get(tabId) || [];
      ...
      if (record) {
        ...
        if (this.debugMode) {
          console.log(`更新标签页[${tabId}]活跃时间: +${elapsedTime}ms, 节点=${lastNodeId}`);
        }
      }
    } catch (error) {
   ...
   * 查找与URL匹配的待处理导航
   */
  private getPendingNavigationForUrl(url: string, tabId?: number): PendingNavigation | null {
    // 标准化URL
    const normalizedUrl = this.normalizeUrl(url);
    ...
    if (this.pendingNavigations.has(normalizedUrl)) {
      ...
      // 找到最近的尚未过期的导航
      const now = Date.now();
      const foundNavigation = navigations.find(nav => 
        nav.expiresAt > now && 
        (!tabId || nav.isNewTab || nav.sourceTabId === tabId || nav.targetTabId === tabId)
      );
   ...
   * 根节点是指没有父节点的导航
   */
  private shouldBeRootNavigation(navigationType: NavigationType, url: string): boolean {
  ...
  private shouldUseReferrerForParent(record: NavigationRecord): boolean {
    ...
    return record.navigationType === 'javascript' || 
          !record.parentId || 
          record.parentId === '' ||
