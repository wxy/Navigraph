...
export function registerSettingsHandlers(messageService: BackgroundMessageService): void {
  ...
  messageService.registerHandler('getSettings', (
    ...
    if (!ctx) {
      logger.error(_('settings_context_failed', '创建消息上下文失败'));
      return false;
    }
    ...
    try {
      ...
      settingsService.getSettings()
        ...
        .catch((error: Error) => {
          logger.error(_('settings_get_error', '获取设置时出错'), error);
          ctx.error(_('settings_get_failed', '获取设置失败: {0}'), error.message);
        });
    } catch (error) {
      logger.error(_('settings_handler_error', '处理{0}请求时出错'), 'getSettings');
      ctx.error(_('settings_request_failed', '处理请求失败: {0}'), error instanceof Error ? error.message : String(error));
    }
  ...
  messageService.registerHandler('saveSettings', (
    ...
    if (!settings) {
      ctx.error(_('settings_missing_data', '缺少设置数据'));
      return false;
    }
    ...
    try {
      ...
      settingsService.updateSettings(settings)
        ...
        .catch((error: Error) => {
          logger.error(_('settings_save_error', '保存设置时出错'), error);
          ctx.error(_('settings_save_failed', '保存设置失败: {0}'), error.message);
        });
    } catch (error) {
      logger.error(_('settings_handler_error', '处理{0}请求时出错'), 'saveSettings');
      ctx.error(_('settings_request_failed', '处理请求失败: {0}'), error instanceof Error ? error.message : String(error));
    }
  ...
  messageService.registerHandler('resetSettings', (
    ...
    if (!message) {
      ctx.error(_('settings_invalid_request', '无效的请求'));
      return false;
    }
    ...
    settingsService.resetSettings()
      .then(() => ctx.success())
      .catch(error => {
        logger.error(_('settings_reset_error', '重置设置时出错'), error);
        ctx.error(_('settings_reset_failed', '重置设置失败: {0}'), error instanceof Error ? error.message : String(error));
      });
    
    return true; // 需要异步响应
  });
  
  logger.log(_('settings_handlers_registered', '设置相关消息处理程序已注册'));
}