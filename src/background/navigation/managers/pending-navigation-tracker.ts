...
export class PendingNavigationTracker {
  ...
  constructor(expirationTime?: number) {
    ...
    logger.log(_('pending_nav_tracker_initialized', '待处理导航追踪器初始化完成'));
  }
  ...
  addLinkNavigation(linkInfo: LinkClickInfo): void {
    ...
    // 添加日志记录
    logger.log(_('pending_nav_tracker_link_added', '添加链接点击导航: {0} -> {1}'), sourceUrl, targetUrl);
  }
  ...
  addFormSubmission(tabId: number, formInfo: FormSubmitInfo): void {
    ...
    // 添加日志记录
    logger.log(_('pending_nav_tracker_form_added', '添加表单提交导航: 标签页={0}, 目标={1}'), tabId.toString(), formInfo.formAction);
  }
  ...
  addJsNavigation(tabId: number, jsNavInfo: {
    ...
    // 添加日志记录
    logger.log(_('pending_nav_tracker_js_added', '添加JS导航: 标签页={0}, 从 {1} 到 {2}'), tabId.toString(), jsNavInfo.sourceUrl, jsNavInfo.targetUrl);
  }
  ...
  public addRedirectNavigation(redirectInfo: {
    ...
    logger.log(_('pending_nav_tracker_redirect_added', '添加重定向导航记录: 从 {0} 到 {1}, 标签页={2}'),
  ...
  cleanupExpiredNavigations(): number {
    try {
      ...
      // 如果有导航被删除且在调试模式，记录日志
      if (totalRemoved > 0) {
        logger.log(_('pending_nav_tracker_cleanup_complete', '清理了 {0} 个过期的待处理导航记录'), totalRemoved.toString());
      }
      
      return totalRemoved;
    } catch (error) {
      logger.error(_('pending_nav_tracker_cleanup_failed', '清理过期导航失败: {0}'), error instanceof Error ? error.message : String(error));
      return 0;
    }
  }
  ...
  reset(): void {
    ...
    logger.log(_('pending_nav_tracker_reset', '待处理导航追踪器状态已重置'));
  }
  ...
  clearTabNavigations(tabId: number): void {
    ...
    // 添加日志记录
    logger.log(_('pending_nav_tracker_tab_cleared', '已清理标签页 {0} 的导航记录'), tabId.toString());
  }
