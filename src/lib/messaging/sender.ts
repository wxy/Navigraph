...
export function sendMessage<T extends MessageTarget, A extends string>(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      console.log(`发送消息: ${action} [ID:${requestId}] 至 ${target}`);
...
export function sendMessageWithRetry<T extends MessageTarget, A extends string>(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        if (attempt > 0) {
          console.log(`重试发送消息 (${attempt}/${maxRetries}): ${action}`);
        ...
        console.warn(`发送消息失败 (${attempt}/${maxRetries}):`, error);
        lastError = error;
        
        // 如果是扩展上下文失效错误，不再重试
        if (error instanceof Error && 
            error.message.includes('Extension context invalidated')) {
          console.error('扩展上下文已失效，不再重试');
          break;
        }
      }
    }
