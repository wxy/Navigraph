...
class I18nPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.watchRun.tapAsync('I18nPlugin', (compilation, callback) => {
      ...
      changedFiles.forEach(file => {
        ...
        // 处理翻译文件变化
        else if (file.includes('_locales/') && file.endsWith('/messages.json') && 
  ...
  processI18nFiles() {
      ...
      try {
        ...
        Object.entries(content).forEach(([key, value]) => {
          if (defaultMessages[key]) {
            ...
            // 获取现有源文件列表 - 修改调用方式
            const existingSourceFiles = defaultMessages[key]._sourceFiles || 
            ...
            if (!existingSourceFiles.includes(newSourceFile)) {
              const updatedSourceFiles = [...existingSourceFiles, newSourceFile];
              
              // 更新消息，使用干净的描述和合并的源文件列表
              defaultMessages[key] = {
                message: defaultMessages[key].message,
                description: this.cleanDescription(defaultMessages[key].description),
    ...
    // 3. 保存默认语言文件 (直接替换)
    const defaultLangPath = path.join(tempOutputDir, defaultLang, 'messages.json');
    ...
    try {
      const localeDirs = fs.existsSync(tempOutputDir) ? 
        fs.readdirSync(tempOutputDir).filter(
          dir => fs.statSync(path.join(tempOutputDir, dir)).isDirectory() && dir !== defaultLang
        ) : [];
      ...
      localeDirs.forEach(lang => {
        ...
        Object.entries(defaultMessages).forEach(([key, value]) => {
          const sourceFiles = value._sourceFiles || this.extractSourceFiles(value.description);
          const cleanDesc = value.description ? this.cleanDescription(value.description) : '';
          
          if (existingMessages[key] && existingMessages[key].message) {
            const isUntranslated = existingMessages[key]._untranslated === true || 
        ...
        console.log(`${lang} 语言翻译完成率: ${coverage}% (${translatedCount}/${totalCount})`);
      });
    } catch (error) {
